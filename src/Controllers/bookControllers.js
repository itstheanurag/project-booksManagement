...
const getBook = async (req,res)=>{
    try{
        ...
        let findBook = await book.find(filter).select({_id : 1, title : 1, excerpt : 1, userId : 1, catergory : 1, releasedAt : 1, reviews : 1}).sort({title : 1})
...
const bybookId = async (req,res)=>{
    try{
        ...
        return res.status(200).send({status : false, message : 'Book details', data : details})
    }
    catch(err){
        return res.status(500).send({status : false, message : err.message})
    }
...
const updateBook = async (req,res)=>{
    try{
        ...
        let findBook = await book.findOneAndUpdate({_id : bookId}, {$set : {...data}, updatedAt : Date.now()}, {new : true, upsert : true})
        ...
        if(findBook.isDeleted){
            return res.status(404).send({status : false, message : "this book has been deleted by you"})
        }
        else{
            return res.status(200).send({status : true, message : "Updated successfully", data : findBook})
        }
    }
    catch(err){
        return res.status(500).send({status : false, message : err.message})
...
const deleteById = async (req,res)=>{
    try{
        ...
        if(userId){
            ...
            return res.status(400).send({status : false, message : "User Id must be present in order to perform this action"})
        }

        let findOne = await book.findOne(bookId)
        
        if(!findOne){
            return res.status(404).send({status : false, message : "No book are present with this book Id"})
        }

        if(findOne.userId != userId){
            return res.status(401).send({status : false, message : "This book doesn't belong to you, hence you can't delete it"})
        }
        ...
        if(deleteBook){
            return res.status(400).send({status : false, message : 'Your book has been deleted', data: deleteBook })
        }
